...
angular.module('frontendApp').factory('GeoFiles', ['$http', '$q', '$cacheFactory', function($http, $q, $cacheFactory) {
  ...
  svc.getAreas = function() {
    ...
    $http.get("scripts/geo/ca_polygon.topo.json", {cache: true}).success(function(data, status) {
  ...
  svc.getDistricts = function() {
    ...
    $http.get("scripts/geo/dc_polygon.topo.json", {cache: true}).success(function(data, status) {
